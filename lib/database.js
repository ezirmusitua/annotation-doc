    ...
    handle (doc, parsed) {
        parsed.forEach(word => {
            if (!this.wordToDocMap[ word ]) {
                this.wordToDocMap[ word ] = toSet(new Set())
            }
            this.wordToDocMap[ word ].push(doc);
        });
        return this;
    }

    get (key) {
        return toSet(this.wordToDocMap[ key ]);
    }
...
module.exports = { ContentParser, IndexManager, Executor, setLike: toSet };