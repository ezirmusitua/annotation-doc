...
class IndexManager {
    ...
    async load () {
        this.wordToDocMap = await (new File(Config.index).load());
    }
}
...
class Executor {
    ...
    async constructIndex () {
        ...
        const fileLoadPromises = fileNames.map(async (file) => {
            const content = await (new File(Config.docs + '/' + file)).load();
    ...
    query (content) {
        ...
        // TODO: Fix FileParser dump format
        return docs.reduce(async (res, file) => {
            res.push(await (new File(Config.docs + '/' + file).load()));
            return res;
        }, []);
    }
}
