...
class JsonFile {
    ...
    dump () {
        return JsonFile.dump(this.filePath, this.content).catch(err => {
            throw err;
        });
    }
    ...
    static dump (filePath, content) {
        ...
        return File.write(filePath, contentToDump).catch(err => {
            throw err;
        });
    }
}
...
class NormalFile {
    ...
    read () {
        return File.read(this.filePath, this.content).catch(err => {
            throw err;
        });
    }

    write () {
        return File.write(this.filePath, this.content).catch(err => {
            throw err;
        });
    }
}
...
module.exports = {
    File, JsonFile, DixFile, NormalFile,
};